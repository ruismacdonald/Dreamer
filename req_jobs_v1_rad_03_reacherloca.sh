#!/bin/bash
#SBATCH --job-name=d_v1_r_rad_03_rew
...
#SBATCH --output=/home/ruism/projects/def-rsdjjana/ruism/Dreamer/results/reacherloca_v1_rad_03_rew/%A-%a.out
#SBATCH --error=/home/ruism/projects/def-rsdjjana/ruism/Dreamer/results/reacherloca_v1_rad_03_rew/%A-%a.err

set -e -o pipefail

# Top-level results dir on Lustre
BASE_SAVE_DIR="$HOME/projects/def-rsdjjana/ruism/Dreamer/results/reacherloca_v1_rad_03_rew"
...
RUN_DIR="${SLURM_TMPDIR}/dreamer-v1-r-rad-03-rew-${SLURM_JOB_ID:-0}-${SEED}"
...
python -u "$DREAMER_SRC/dreamer.py" \
  ...
  --algo 'Dreamerv2' \
  --exp-name 'reacherloca_v1_rad_03_rew' \
  --train \
  --loca-all-phases \
