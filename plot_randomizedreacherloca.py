...
def plot(task, ax, runs, methods, args):
  ...
  ax.text(0.75e6, 4000 + 80, "Phase 1", ha="center", va="bottom")
  ax.text(2.25e6, 2000 + 80, "Phase 2", ha="center", va="bottom")
...
def baseline(index, method, ax, runs, args):
  ...
  ax.axhline(4000, label=method, xmin=0,   xmax=0.3636, **kw)  # Phase 1 optimal
  ax.axhline(2000, label=None, xmin=0.3636, xmax=1.0, **kw)  # Phase 2 optimal
  ax.axvline(2000000, label=None, linestyle='dotted', **kw)
...
def legend(fig, mapping=None, **kwargs):
    ...
    bbox = kwargs.pop('bbox_to_anchor', (0.5, 0.12))
