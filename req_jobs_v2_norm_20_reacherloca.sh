#!/bin/bash
#SBATCH --job-name=d_v2_r_norm_20
...
#SBATCH --output=/home/ruism/projects/def-rsdjjana/ruism/Dreamer/results/reacherloca_v2_norm_20/%A-%a.out
#SBATCH --error=/home/ruism/projects/def-rsdjjana/ruism/Dreamer/results/reacherloca_v2_norm_20/%A-%a.err

set -e -o pipefail

# Top-level results dir on Lustre
BASE_SAVE_DIR="$HOME/projects/def-rsdjjana/ruism/Dreamer/results/reacherloca_v2_norm_20"
...
RUN_DIR="${SLURM_TMPDIR}/dreamer-v2-norm-r-20-${SLURM_JOB_ID:-0}-${SEED}"
...
python -u "$DREAMER_SRC/dreamer.py" \
  ...
  --algo 'Dreamerv2' \
  --exp-name 'reacherloca_v2_norm_20' \
  --train \
  --loca-all-phases \
